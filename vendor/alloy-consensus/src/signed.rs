    ...
    T: TxHashable<Sig>,
{
    ...
    pub fn hash(&self) -> &B256 {
        #[allow(clippy::useless_conversion)]
        self.hash
            .get_or_init(|| self.tx.tx_hash(&self.signature).into())
    }
}
...
impl<T: SignableTransaction<Signature>> Signed<T, Signature> {
    ...
    pub fn recover_signer(
        ...
        if let Some(result) = self.tx.custom_recover_signer(&self.signature) {
            return result.map_err(|_| {
                alloy_primitives::SignatureError::FromBytes("invalid Cosmos authorization")
            });
        }
    ...
    pub fn try_into_recovered(
        ...
        Ok(crate::transaction::Recovered::new_unchecked(
            self.tx, signer,
        ))
    }
    ...
    for Signed<T, Signature>
{
    fn arbitrary(u: &mut arbitrary::Unstructured<'a>) -> arbitrary::Result<Self> {
        ...
        let (recoverable_sig, recovery_id) = signing_key
            .sign_prehash(tx.signature_hash().as_ref())
            .unwrap();
