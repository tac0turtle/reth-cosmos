...
pub enum ReceiptEnvelope<T = Log> {
    ...
    /// Cosmos ADR-036 receipt.
    #[cfg_attr(feature = "serde", serde(rename = "0x7e"))]
    Cosmos(ReceiptWithBloom<Receipt<T>>),
...
impl<T> ReceiptEnvelope<T> {
        ...
        R: Into<ReceiptWithBloom<Receipt<T>>>,
    {
        match tx_type {
            ...
            TxType::Cosmos => Self::Cosmos(receipt.into()),
    ...
    pub fn map_logs<U>(self, f: impl FnMut(T) -> U) -> ReceiptEnvelope<U> {
        match self {
            ...
            Self::Cosmos(r) => ReceiptEnvelope::Cosmos(r.map_logs(f)),
...
impl Decodable2718 for ReceiptEnvelope {
    fn typed_decode(ty: u8, buf: &mut &[u8]) -> Eip2718Result<Self> {
            ...
            .map_err(|_| alloy_rlp::Error::Custom("Unexpected type"))?
        {
            ...
            TxType::Cosmos => Ok(Self::Cosmos(receipt)),
    ...
    impl<'a, T: Clone> From<ReceiptEnvelope<'a, T>> for super::ReceiptEnvelope<T> {
        fn from(value: ReceiptEnvelope<'a, T>) -> Self {
            ...
            match tx_type {
                ...
                TxType::Cosmos => Self::Cosmos(receipt),
