...
impl EthereumTxEnvelope<TxEip4844> {
    ...
    pub fn try_into_pooled<T>(
        ...
        match self {
            ...
            Self::Eip1559(tx) => Ok(tx.into()),
            Self::Cosmos(tx) => Ok(tx.into()),
            Self::Eip4844(tx) => Err(ValueError::new(
                tx.into(),
    ...
    pub fn try_into_pooled_eip4844<T>(
        ...
        match self {
            Self::Eip4844(tx) => Ok(EthereumTxEnvelope::Eip4844(
                tx.map(|tx| tx.with_sidecar(sidecar)),
            )),
...
impl<T> EthereumTxEnvelope<T> {
        ...
        T: RlpEcdsaEncodableTx,
    {
        match self {
            ...
            Self::Cosmos(tx) => EthereumTypedTransaction::Cosmos(tx.into_parts().0),
        ...
        T: AsMut<TxEip4844>,
    {
        match self {
            ...
            Self::Cosmos(tx) => &mut tx.tx_mut().operation.input,
...
impl<T> EthereumTypedTransaction<TxEip4844Variant<T>> {
    ...
    pub fn strip_eip4844_sidecar_into<U>(
        ...
        match self {
            ...
            Self::Cosmos(tx) => (EthereumTypedTransaction::Cosmos(tx), None),
            Self::Eip4844(tx) => {
...
pub enum EthereumTxEnvelope<Eip4844> {
    ...
    /// Cosmos ADR-036 authorization (experimental local chain).
    #[envelope(ty = 126)]
    #[serde(with = "crate::transaction::cosmos::signed_serde")]
    Cosmos(Signed<TxCosmos>),
    ...
    T: RlpEcdsaEncodableTx,
{
    fn from(v: Signed<T>) -> Self {
        ...
        match typed {
            ...
            EthereumTypedTransaction::Cosmos(tx_cosmos) => {
                let tx = Signed::new_unchecked(tx_cosmos, sig, hash);
                Self::Cosmos(tx)
            }
...
impl<Eip4844> EthereumTxEnvelope<Eip4844> {
    ...
    pub fn map_eip4844<U>(self, f: impl FnMut(Eip4844) -> U) -> EthereumTxEnvelope<U> {
        match self {
            ...
            Self::Cosmos(tx) => EthereumTxEnvelope::Cosmos(tx),
    ...
    pub fn try_map_eip4844<U, E>(
        ...
        match self {
            ...
            Self::Cosmos(tx) => Ok(EthereumTxEnvelope::Cosmos(tx)),
...
impl<Eip4844: RlpEcdsaEncodableTx> EthereumTxEnvelope<Eip4844> {
    ...
    pub fn try_into_eip2930(self) -> Result<Signed<TxEip2930>, ValueError<Self>> {
        match self {
            ...
            _ => Err(ValueError::new_static(
                self,
                "Expected EIP-2930 transaction",
            )),
        }
    }
    ...
    pub fn eip2718_encoded_length(&self) -> usize {
        match self {
            ...
            Self::Cosmos(t) => t.eip2718_encoded_length(),
    ...
    Eip4844: RlpEcdsaEncodableTx + SignableTransaction<Signature>,
{
    fn recover_signer(&self) -> Result<alloy_primitives::Address, crate::crypto::RecoveryError> {
        match self {
            ...
            Self::Cosmos(tx) => tx.tx().verify(tx.signature()),
    ...
    impl<'a, T: Clone> From<&'a super::EthereumTxEnvelope<T>> for EthereumTxEnvelope<'a, T> {
        fn from(value: &'a super::EthereumTxEnvelope<T>) -> Self {
            match value {
                ...
                super::EthereumTxEnvelope::Cosmos(tx) => Self {
                    signature: *tx.signature(),
                    transaction:
                        crate::serde_bincode_compat::transaction::EthereumTypedTransaction::Cosmos(
                            alloy_rlp::encode(tx.tx()).into(),
                        ),
                },
    ...
    impl<'a, T: Clone> From<EthereumTxEnvelope<'a, T>> for super::EthereumTxEnvelope<T> {
        fn from(value: EthereumTxEnvelope<'a, T>) -> Self {
            ...
            match transaction {
                ...
                EthereumTypedTransaction::Cosmos(tx) => Signed::new_unhashed(tx, signature).into(),
...
mod tests {
    ...
    fn test_decode_live_1559_tx() {
        ...
        assert_eq!(
            tx.tx().to,
            TxKind::Call(address!("D9e1459A7A482635700cBc20BBAF52D495Ab9C96"))
        );
    ...
    fn test_tx_type_from_conversions() {
        ...
        assert!(matches!(
            TxEnvelope::from(eip2930_tx),
            TxEnvelope::Eip2930(_)
        ));
        assert!(matches!(
            TxEnvelope::from(eip1559_tx),
            TxEnvelope::Eip1559(_)
        ));
        assert!(matches!(
            TxEnvelope::from(eip4844_variant),
            TxEnvelope::Eip4844(_)
        ));
        assert!(matches!(
            TxEnvelope::from(eip7702_tx),
    ...
    fn test_recover_with_buf_eip1559() {
        ...
        assert_eq!(
            from_standard,
            address!("001e2b7dE757bA469a57bF6b23d982458a07eFcE")
        );
