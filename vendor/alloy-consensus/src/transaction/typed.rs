...
impl<Eip4844> From<EthereumTxEnvelope<Eip4844>> for EthereumTypedTransaction<Eip4844> {
    fn from(envelope: EthereumTxEnvelope<Eip4844>) -> Self {
        match envelope {
            ...
            EthereumTxEnvelope::Cosmos(tx) => Self::Cosmos(tx.strip_signature()),
...
impl<Eip4844> EthereumTypedTransaction<Eip4844> {
    ...
    pub fn map_eip4844<U>(self, mut f: impl FnMut(Eip4844) -> U) -> EthereumTypedTransaction<U> {
        match self {
            ...
            Self::Cosmos(tx) => EthereumTypedTransaction::Cosmos(tx),
    ...
    pub fn into_envelope(self, signature: Signature) -> EthereumTxEnvelope<Eip4844> {
        match self {
            ...
            Self::Cosmos(tx) => EthereumTxEnvelope::Cosmos(tx.into_signed(signature)),
...
impl<Eip4844: RlpEcdsaEncodableTx> EthereumTypedTransaction<Eip4844> {
    ...
    pub fn tx_hash(&self, signature: &Signature) -> TxHash {
        match self {
            ...
            Self::Cosmos(tx) => tx.tx_hash(signature),
    ...
    for EthereumTypedTransaction<Eip4844>
{
    fn rlp_header_signed(&self, signature: &Signature) -> alloy_rlp::Header {
        if let Self::Cosmos(tx) = self {
            return tx.rlp_header_signed(signature);
        }
        alloy_rlp::Header {
            list: true,
            payload_length: self.rlp_encoded_fields_length()
                + signature.rlp_rs_len()
                + alloy_rlp::Encodable::length(&signature.v()),
        }
    }

    fn rlp_encode_signed(&self, signature: &Signature, out: &mut dyn BufMut) {
        if let Self::Cosmos(tx) = self {
            return tx.rlp_encode_signed(signature, out);
        }
    ...
    fn rlp_encoded_fields_length(&self) -> usize {
        match self {
            ...
            Self::Cosmos(tx) => tx.rlp_encoded_fields_length(),
    ...
    fn rlp_encode_fields(&self, out: &mut dyn alloy_rlp::BufMut) {
        match self {
            ...
            Self::Cosmos(tx) => tx.rlp_encode_fields(out),
    ...
    fn eip2718_encode_with_type(&self, signature: &Signature, _ty: u8, out: &mut dyn BufMut) {
        match self {
            ...
            Self::Cosmos(tx) => tx.eip2718_encode_with_type(signature, tx.ty(), out),
    ...
    fn eip2718_encode(&self, signature: &Signature, out: &mut dyn BufMut) {
        match self {
            ...
            Self::Cosmos(tx) => tx.eip2718_encode(signature, out),
    ...
    fn network_encode_with_type(&self, signature: &Signature, _ty: u8, out: &mut dyn BufMut) {
        match self {
            ...
            Self::Cosmos(tx) => tx.network_encode_with_type(signature, tx.ty(), out),
    ...
    fn network_encode(&self, signature: &Signature, out: &mut dyn BufMut) {
        match self {
            ...
            Self::Cosmos(tx) => tx.network_encode(signature, out),
    ...
    fn tx_hash_with_type(&self, signature: &Signature, _ty: u8) -> TxHash {
        match self {
            ...
            Self::Cosmos(tx) => tx.tx_hash_with_type(signature, tx.ty()),
    ...
    for EthereumTypedTransaction<Eip4844>
{
    fn custom_recover_signer(
        &self,
        signature: &Signature,
    ) -> Option<Result<alloy_primitives::Address, crate::crypto::RecoveryError>> {
        match self {
            Self::Cosmos(tx) => tx.custom_recover_signer(signature),
            _ => None,
        }
    }
    fn signature_hash(&self) -> alloy_primitives::B256 {
        match self {
            Self::Cosmos(tx) => tx.signature_hash(),
            _ => alloy_primitives::keccak256(self.encoded_for_signing()),
        }
    }
    ...
    fn set_chain_id(&mut self, chain_id: ChainId) {
        match self {
            ...
            Self::Cosmos(tx) => tx.set_chain_id(chain_id),
    ...
    fn encode_for_signing(&self, out: &mut dyn BufMut) {
        match self {
            ...
            Self::Cosmos(tx) => tx.encode_for_signing(out),
    ...
    fn payload_len_for_signature(&self) -> usize {
        match self {
            ...
            Self::Cosmos(tx) => tx.payload_len_for_signature(),
...
pub(crate) mod serde_bincode_compat {
    ...
    pub enum EthereumTypedTransaction<'a, Eip4844: Clone = crate::transaction::TxEip4844> {
        ...
        /// Canonical unsigned Cosmos bytes.
        Cosmos(alloy_primitives::Bytes),
        ...
        for EthereumTypedTransaction<'a, T>
    {
        fn from(value: &'a super::EthereumTypedTransaction<T>) -> Self {
            match value {
                ...
                super::EthereumTypedTransaction::Cosmos(tx) => {
                    Self::Cosmos(alloy_rlp::encode(tx).into())
                }
    ...
    impl<'a, T: Clone> From<EthereumTypedTransaction<'a, T>> for super::EthereumTypedTransaction<T> {
        fn from(value: EthereumTypedTransaction<'a, T>) -> Self {
            match value {
                ...
                EthereumTypedTransaction::Cosmos(tx) => {
                    Self::Cosmos(alloy_rlp::decode_exact(&tx).expect("stored Cosmos transaction"))
                }
...
mod tests {
    ...
    fn test_decode_unsigned_invalid_type() {
        ...
        if let Err(err) = result {
            assert!(matches!(
                err,
                alloy_eips::eip2718::Eip2718Error::UnexpectedType(0x99)
            ));
        }
    }
...
impl<T> From<TxCosmos> for EthereumTypedTransaction<T> {
    fn from(tx: TxCosmos) -> Self {
        Self::Cosmos(tx)
    }
}
