...
impl NetworkTransactionBuilder<Ethereum> for TransactionRequest {
    ...
    fn complete_type(&self, ty: TxType) -> Result<(), Vec<&'static str>> {
        match ty {
            ...
            TxType::Cosmos => Err(vec!["cosmos_sendRawTransaction authorization"]),
...
mod tests {
    ...
    fn test_fail_when_sidecar_and_access_list() {
        ...
        assert!(matches!(
            error.error,
            TransactionBuilderError::InvalidTransactionRequest(_, _)
        ));
    }
    ...
    fn test_tx_response_into_req() {
        ...
        assert_eq!(
            TransactionBuilder::max_priority_fee_per_gas(&req).unwrap(),
            123
        );
