    ...
    Eip4844: Compact + RlpEcdsaEncodableTx,
{
        ...
        B: BufMut + AsMut<[u8]>,
    {
        ...
        match self {
            ...
            Self::Cosmos(tx) => tx.to_compact(buf),
    ...
    fn from_compact(buf: &[u8], identifier: usize) -> (Self, &[u8]) {
        ...
        match tx_type {
            ...
            TxType::Cosmos => {
                let (tx, buf) = TxCosmos::from_compact(buf, buf.len());
                (Self::Cosmos(tx), buf)
            }
            TxType::Eip2930 => {
...
impl Compact for TxCosmos {
    ...
    fn from_compact(mut buf: &[u8], _: usize) -> (Self, &[u8]) {
        let tx = alloy_rlp::Decodable::decode(&mut buf).expect("corrupt stored Cosmos transaction");
        (tx, buf)
    }
}
