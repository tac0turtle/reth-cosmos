...
impl<Eip4844: AsRef<TxEip4844>> FromTxWithEncoded<EthereumTxEnvelope<Eip4844>> for TxEnv {
    fn from_encoded_tx(tx: &EthereumTxEnvelope<Eip4844>, caller: Address, encoded: Bytes) -> Self {
        match tx {
            ...
            EthereumTxEnvelope::Cosmos(tx) => Self::from_recovered_tx(tx.tx(), caller),
...
impl<Eip4844: AsRef<TxEip4844>> FromRecoveredTx<EthereumTxEnvelope<Eip4844>> for TxEnv {
    fn from_recovered_tx(tx: &EthereumTxEnvelope<Eip4844>, sender: Address) -> Self {
        match tx {
            ...
            EthereumTxEnvelope::Cosmos(tx) => Self::from_recovered_tx(tx.tx(), sender),
...
impl FromRecoveredTx<alloy_consensus::TxCosmos> for TxEnv {
    fn from_recovered_tx(tx: &alloy_consensus::TxCosmos, caller: Address) -> Self {
        let op = &tx.operation;
        Self {
            // EVM fee and execution rules are EIP-1559; receipts keep outer type 0x7e.
            tx_type: 2,
            caller,
